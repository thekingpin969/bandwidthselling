...
async function stopApp(ctx: any) {
    try {
        ...
        keyboard.push([{ text: 'pause all', callback_data: `pauseModules:all-${appName}` }]);
        for (let i = 0; i < modules.length; i += 2) {
            const row = [];
            if (modules[i]) row.push({ text: modules[i].split('-')[1], callback_data: `pauseModules:${modules[i]}` });
            if (modules[i + 1]) row.push({ text: modules[i + 1].split('-')[1], callback_data: `pauseModules:${modules[i + 1]}` });
            keyboard.push(row);
        }
